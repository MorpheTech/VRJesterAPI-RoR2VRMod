...
namespace com.calicraft.vrjester.utils.demo
{
	...
	using Level = net.minecraft.world.level.Level;
	...
	public class SpawnParticles
	{
		...
		public static void moveParticles(SimpleParticleType type, Vec3 position, Vec3 direction, double speedMultiplier)
		{
			Random rand;
			Vec3 velocity;
			LocalPlayer player = VrJesterApi.MCI.player;
			...
			if (player.getCommandSenderWorld().isClientSide())
			{
				...
				{
					rand = new Random();
					velocity = direction.add((rand.nextGaussian() * 0.005D), (rand.nextGaussian() * 0.005D), (rand.nextGaussian() * 0.005D));
					velocity = velocity.multiply((speedMultiplier), (speedMultiplier), (speedMultiplier));
					clientWorld.addParticle(type, position.x, position.y, position.z, velocity.x, velocity.y, velocity.z);
				}
			}
		}
			...
			if (player.getCommandSenderWorld().isClientSide())
			{
				...
				for (int i = 0; i < 100; i++)
				{
					...
					if (type == ParticleTypes.BUBBLE)
					{
						...
						clientWorld.addParticle(type, position.x, position.y, position.z, motionX, motionY, motionZ);
					}
					else
					{
		...
		public static void moveParticles(SimpleParticleType type, double speed)
		{
			...
			if (player.getCommandSenderWorld().isClientSide())
			{
				...
				for (int i = 0; i < 20; i++)
				{
					clientWorld.addParticle(type, newPos.x, newPos.y, newPos.z, (dir.x + speed), (dir.y + speed), (dir.z + speed));
				}
			}
		}
		...
		public static void createParticles(SimpleParticleType type)
		{
			...
			if (player.getCommandSenderWorld().isClientSide())
			{
				...
				for (int i = 0; i < 20; i++)
				{
					...
					clientWorld.addParticle(type, newPos.x, newPos.y, newPos.z, motionX, motionY, motionZ);
				}
			}
		}
		...
		public static void createParticles(Vec3 position)
		{
			...
			if (player.getCommandSenderWorld().isClientSide())
			{
				...
				for (int i = 0; i < 5; i++)
				{
					clientWorld.addParticle(ParticleTypes.BUBBLE, position.x, position.y, position.z, (0.0D), (0.0D), (0.0D));
				}
			}
		}
	}
